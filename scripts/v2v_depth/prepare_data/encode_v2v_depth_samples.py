...
``fastvideo/pipelines/stages/v2v_depth_conditioning.py`` mirrors the crop,
resize and depth encoding below so validation reproduces this cache. Change one
and change the other: a mismatch shifts the control signal against the frame
without raising anything, it only makes the metrics worse.
