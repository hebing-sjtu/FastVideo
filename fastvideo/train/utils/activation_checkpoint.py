    ...
    for transformer_block_name in _TRANSFORMER_BLOCK_NAMES:
        ...
        for index, (layer_id, block) in enumerate(blocks.named_children()):
            if n_layer is None or index % n_layer == 0:
                ...
                # Keep the default NO_REENTRANT impl. REENTRANT + FSDP + LoRA
                # severs the graph: loss has no grad_fn and backward raises
                # ``element 0 of tensors does not require grad``. Validation
